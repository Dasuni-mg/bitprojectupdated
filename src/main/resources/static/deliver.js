...
function btnInnerAddMC() {
    ...
    for (var index in reservation.reservationHasServiceList) {
        if (reservation.reservationHasServiceList[index].menu_id != null) {
            console.log("LIST ",reservation.reservationHasServiceList[index].menu_id.menuname)

            if (reservation.reservationHasServiceList[index].menu_id.menuname
                == reservationHasService.menu_id.menuname
            ) {
                resv = true;
                break;
            }
...
function getErrors() {
    ...
    if (reservation.customer_id == null) {
        errors = errors + "\n" + "Customer Tp Not Entered ";
        cmbRegCustomer.style.border = invalid;
    } else addvalue = 1;
...
function btnAddMC() {
    if (getErrors() == "") {
        if (cmbRegCustomer.value == "" || txtDiscountRatio.value == "" || txtAdvanceAmount.value=="") {
            swal({
    ...
    swal({
     ...
     var responseReservation = httpRequest("/reservation", "POST", reservation);
            ...
            customerpayment.reservationamount = reservation.lastprice;
             customerpayment.balanceamount= reservation.balanceamount;
            customerpayment.paidamount= parseFloat(0).toFixed(2);
            // customerpayment.currentamount =reservation.currentamount;
