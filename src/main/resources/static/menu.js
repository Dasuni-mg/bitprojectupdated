...
function viewmenu(mnu, rowno) {
    ...
    tdAddDate.innerHTML= menu.addeddate;
    cmbEmployee.innerHTML=menu.employee_id;
    tdmStatus.innerHTML= menu.menustatus_id.name;
...
function loadForm() {
    ...
    fillCombo(cmbMstatus, "", menustatuses, "name", "Available");
    menu.menustatus_id = JSON.parse(cmbMstatus.value);
    cmbMstatus.disabled = true;

    //fill and auto select autobind
    fillCombo(cmbEmployee,"",employees,"callingname",session.getObject('activeuser').employeeId.callingname);
    menu.employee_id = JSON.parse(cmbEmployee.value);
    ...
    menu.addeddate = dateAddedDate.value;
...
function savedata() {
    ...
    swal({
        ...
        text:
            ...
            "\n price : " + menu.price +
            "\n Added Date : " + menu.addeddate +

            "\n Menu Status : " + menu.menustatus_id.name,
        icon: "warning",
        buttons: true,
        dangerMode: true,
    }).then((willDelete) => {
...
function filldata(mnu) {
    ...
    dateAddedDate.value= menu.addeddate ;

    fillCombo(cmbMstatus, "Select Menu Status", menustatuses, "name", menu.menustatus_id.name);
